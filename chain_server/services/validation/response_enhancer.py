...
class ResponseEnhancer:
    ...
    async def enhance_response(
        ...
        try:
            ...
            if auto_fix and (not validation_result.is_valid or validation_result.score < 0.99 or len(validation_result.issues) > 0):
    ...
    async def _apply_enhancements(
        ...
        for issue in sorted_issues:
            try:
                ...
                logger.warning(f"Error applying enhancement for issue {issue.message}: {e}")
        
        return enhanced_response, improvements
    
    async def _fix_formatting_issue(self, response: str, issue: ValidationIssue) -> Tuple[str, str]:
    ...
    async def _fix_content_issue(self, response: str, issue: ValidationIssue) -> Tuple[str, str]:
        ...
        if issue.field == "content_repetition":
            ...
            # Fix excessive caps
            enhanced_response = re.sub(r'\b[A-Z]{5,}\b', lambda m: m.group(0).title(), enhanced_response)
            improvement = "Fixed excessive capitalization"
        
        return enhanced_response, improvement
    
    async def _fix_completeness_issue(self, response: str, issue: ValidationIssue) -> Tuple[str, str]:
    ...
    async def _fix_security_issue(self, response: str, issue: ValidationIssue) -> Tuple[str, str]:
        """Fix security issues."""
        enhanced_response = response
        improvement = ""
        ...
        if issue.field == "data_privacy":
            # Mask sensitive data
            enhanced_response = re.sub(r'\b\d{4}-\d{4}-\d{4}-\d{4}\b', '****-****-****-****', enhanced_response)
            enhanced_response = re.sub(r'\b\d{3}-\d{2}-\d{4}\b', '***-**-****', enhanced_response)
            enhanced_response = re.sub(
