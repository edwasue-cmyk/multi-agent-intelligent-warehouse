...
class EvidenceCollector:
    ...
    async def collect_evidence(self, context: EvidenceContext) -> List[Evidence]:
        ...
        try:
            ...
            logger.info(f"Collected {len(evidence_list)} pieces of evidence for query: {context.query[:50]}...")
    ...
    async def _collect_equipment_evidence(self, context: EvidenceContext) -> List[Evidence]:
        ...
        try:
            ...
            equipment_entities = {
                k: v for k, v in context.entities.items() 
            ...
            if self.tool_discovery:
                ...
                for tool in equipment_tools[:3]:  # Limit to 3 tools
                    try:
                        ...
                        # Execute tool
                        result = await self.tool_discovery.execute_tool(tool.tool_id, arguments)
                        ...
                        logger.error(f"Error collecting equipment evidence from tool {tool.name}: {e}")
            ...
            if self.hybrid_retriever:
                try:
                    ...
                    retrieval_results = await self.hybrid_retriever.search(search_context)
                    
                    if retrieval_results and (retrieval_results.structured_results or retrieval_results.vector_results):
                        ...
                        evidence = Evidence(
                            ...
                            metadata={
                                ...
                                "result_count": len(retrieval_results.structured_results) + len(retrieval_results.vector_results)
                            },
                    ...
                    logger.error(f"Error collecting equipment evidence from retriever: {e}")
    ...
    async def _collect_operations_evidence(self, context: EvidenceContext) -> List[Evidence]:
        ...
        try:
            ...
            operations_entities = {
                k: v for k, v in context.entities.items() 
                if k in ['task_id', 'user_id', 'worker_id', 'shift', 'zone', 'operation']
            }
            ...
            if self.tool_discovery:
                ...
                for tool in operations_tools[:2]:  # Limit to 2 tools
                    try:
                        arguments = self._prepare_operations_tool_arguments(tool, operations_entities)
                        result = await self.tool_discovery.execute_tool(tool.tool_id, arguments)
                        ...
                        logger.error(f"Error collecting operations evidence from tool {tool.name}: {e}")
    ...
    async def _collect_safety_evidence(self, context: EvidenceContext) -> List[Evidence]:
        ...
        try:
            ...
            safety_entities = {
                k: v for k, v in context.entities.items() 
                if k in ['incident_id', 'safety_type', 'severity', 'location', 'procedure']
            }
            ...
            if self.tool_discovery:
                ...
                for tool in safety_tools[:2]:  # Limit to 2 tools
                    try:
                        arguments = self._prepare_safety_tool_arguments(tool, safety_entities)
                        result = await self.tool_discovery.execute_tool(tool.tool_id, arguments)
                        ...
                        logger.error(f"Error collecting safety evidence from tool {tool.name}: {e}")
    ...
    async def _collect_historical_evidence(self, context: EvidenceContext) -> List[Evidence]:
    ...
    async def _collect_user_context_evidence(self, context: EvidenceContext) -> List[Evidence]:
    ...
    async def _collect_system_context_evidence(self, context: EvidenceContext) -> List[Evidence]:
    ...
    def _prepare_equipment_tool_arguments(self, tool, entities: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for param_name in tool.parameters.get('properties', {}):
            ...
            elif param_name in ['asset_id', 'equipment_id'] and 'equipment_id' in entities:
    ...
    def _prepare_operations_tool_arguments(self, tool, entities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _prepare_safety_tool_arguments(self, tool, entities: Dict[str, Any]) -> Dict[str, Any]:
    ...
    async def _score_and_rank_evidence(self, evidence_list: List[Evidence], context: EvidenceContext) -> List[Evidence]:
        """Score and rank evidence by relevance and quality."""
        try:
            # Calculate relevance scores
            for evidence in evidence_list:
                evidence.relevance_score = await self._calculate_relevance_score(evidence, context)
    ...
    async def _calculate_relevance_score(self, evidence: Evidence, context: EvidenceContext) -> float:
    ...
    def _update_collection_stats(self, evidence_list: List[Evidence]) -> None:
        ...
        try:
            ...
            if evidence_list:
                ...
                self.collection_stats["average_confidence"] = total_confidence / len(evidence_list)
            
        except Exception as e:
            logger.error(f"Error updating collection stats: {e}")
    
    async def synthesize_evidence(self, evidence_list: List[Evidence], context: EvidenceContext) -> Dict[str, Any]:
        ...
        try:
            ...
            for evidence in evidence_list:
                ...
                synthesis["evidence_summary"]["evidence_by_type"][evidence_type] = \
                    synthesis["evidence_summary"]["evidence_by_type"].get(evidence_type, 0) + 1
                
                # Count by source
                source = evidence.source.value
                synthesis["evidence_summary"]["evidence_by_source"][source] = \
                    synthesis["evidence_summary"]["evidence_by_source"].get(source, 0) + 1
            ...
            # Calculate average confidence
            total_confidence = sum(e.confidence for e in evidence_list)
            synthesis["evidence_summary"]["average_confidence"] = total_confidence / len(evidence_list)
            
            # Generate recommendations based on evidence
            synthesis["recommendations"] = await self._generate_evidence_recommendations(evidence_list, context)
    ...
    async def _generate_evidence_recommendations(self, evidence_list: List[Evidence], context: EvidenceContext) -> List[str]:
        """Generate recommendations based on evidence."""
        recommendations = []
        ...
        try:
            ...
            equipment_evidence = [e for e in evidence_list if e.evidence_type == EvidenceType.EQUIPMENT_DATA]
            safety_evidence = [e for e in evidence_list if e.evidence_type == EvidenceType.SAFETY_DATA]
            
            # Generate contextual recommendations
            if high_confidence_count < 2:
                recommendations.append("Consider gathering additional evidence for higher confidence")
            
            if equipment_evidence and any('maintenance' in str(e.content).lower() for e in equipment_evidence):
                recommendations.append("Schedule maintenance for equipment showing issues")
