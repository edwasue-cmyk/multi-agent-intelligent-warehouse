...
from chain_server.services.mcp import (
    ...
    ToolValidationService, ErrorHandlingService, MCPManager,
    ExecutionContext, RoutingContext, QueryComplexity
)
...
class MCPIntentClassifier:
    ...
    async def classify_intent_with_mcp(self, message: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.tool_routing:
                ...
                mcp_intent = routing_suggestions.primary_agent if routing_suggestions else basic_intent
            else:
            ...
            return {
                ...
                "discovered_tools": await self.tool_discovery.get_tools_for_intent(mcp_intent)
            }
    ...
    def _classify_intent_basic(self, message: str) -> str:
        ...
        operations_score = sum(1 for keyword in self.OPERATIONS_KEYWORDS if keyword in message_lower)
        equipment_score = sum(1 for keyword in self.EQUIPMENT_KEYWORDS if keyword in message_lower)
    ...
    def _assess_query_complexity(self, message: str) -> QueryComplexity:
        ...
        # Simple queries
        if len(message.split()) <= 5 and not any(word in message_lower for word in ["and", "or", "but", "also", "additionally"]):
            return QueryComplexity.SIMPLE
        
        # Complex queries with multiple intents or conditions
        if any(word in message_lower for word in ["and", "or", "but", "also", "additionally", "however", "while", "when", "if"]):
...
class MCPPlannerGraph:
    ...
    async def _mcp_route_intent(self, state: MCPWarehouseState) -> MCPWarehouseState:
        ...
        try:
            ...
            logger.info(f"MCP Intent classified as: {intent_result['intent']} with confidence: {intent_result['confidence']}")
    ...
    async def _mcp_equipment_agent(self, state: MCPWarehouseState) -> MCPWarehouseState:
        ...
        try:
            ...
            latest_message = state["messages"][-1]
            message_text = latest_message.content if isinstance(latest_message, HumanMessage) else str(latest_message.content)
            ...
            execution_plan = await self.tool_binding.create_execution_plan(execution_context)
            ...
            logger.info(f"MCP Equipment agent processed request with confidence: {response.get('confidence', 0)}")
    ...
    async def _mcp_operations_agent(self, state: MCPWarehouseState) -> MCPWarehouseState:
        ...
        try:
            ...
            logger.info(f"MCP Operations agent processed request with confidence: {response.get('confidence', 0)}")
    ...
    async def _mcp_safety_agent(self, state: MCPWarehouseState) -> MCPWarehouseState:
        ...
        try:
            ...
            logger.info(f"MCP Safety agent processed request with confidence: {response.get('confidence', 0)}")
    ...
    async def _mcp_synthesize_response(self, state: MCPWarehouseState) -> MCPWarehouseState:
    ...
    async def _process_mcp_equipment_query(self, query: str, session_id: str, context: Dict, mcp_results: Any) -> Dict[str, Any]:
    ...
    async def _process_mcp_operations_query(self, query: str, session_id: str, context: Dict, mcp_results: Any) -> Dict[str, Any]:
    ...
    async def _process_mcp_safety_query(self, query: str, session_id: str, context: Dict, mcp_results: Any) -> Dict[str, Any]:
